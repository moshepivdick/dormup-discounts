// DISABLED: Discount confirmation disabled since codes are now client-side only
// Partners can no longer confirm codes via API since codes are not stored in database
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  return res.status(410).json({
    ...
    message: 'Discount codes are now generated client-side and cannot be confirmed via API.',
  });
};
